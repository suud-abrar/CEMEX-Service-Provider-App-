class Tablereg {
  ...
  float x, y;
  float startY;
  float scrollHeight = 475;
  float scrollBrickHeight = 63;
  float scrollBrickY;
  Tablereg (String headers [], float x, float y, Button back) {
    this.x = x;
    this.y = y;
  ...
  void addRow (String l1, String l2, String l3) {
    Rows row = new Rows (str (rows.length + 1), l1, l2, l3);
    rows = (Rows []) append (rows, row);
  }
  void draw () {
